...
public abstract class NetIpBase
{
  ...
  public void receive(Message msg, MacAddress lastHop, byte macId, boolean promisc, boolean bcast)
  {
    ...
    {
      log.info("receive t="+JistAPI.getTime()/(float)Constants.SECOND +"\tfrom="+lastHop+"\ton="+macId+"\tdata="+msg);
    }
  ...
  public void send(NetMessage.Ip msg, int interfaceId, MacAddress nextHop) 
  {
    ...
    */
    ...
    {
      log.debug("queue t="+JistAPI.getTime()/(float)Constants.SECOND+"\tto="+nextHop+"\ton="+interfaceId+"\tdata="+msg+"\tprotocol="+msg.getProtocol());
    }
  ...
  public void pump(int interfaceId)
  {
    ...
    else
    {
      ...
      if(log.isInfoEnabled())
      {
        log.info("send t="+JistAPI.getTime()/(float)Constants.SECOND+"\tto="+qmsg.getNextHop()+"\tdata="+ip);
      }
      JistAPI.sleep(Constants.NET_DELAY);
