...
public abstract class AppCbrBase implements AppInterface, NetInterface.NetHandler, StatsCollector {
	...
	public static final int CBR_MESSAGE_SIZE = 64;
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority, byte ttl) {
		...
		if (msg instanceof AppCbrBase.MessageCbr) {
			...
			//	double timeCurrent = JistAPI.getTime()/Constants.SECOND;
				...
				/*if( timeCurrent>100 && timeCurrent<150)
				{
					stats100.put(CBR_STAT_SENT+"_"+((int)timeCurrent), Long.toString(sentPackets));
					stats100.put(CBR_STAT_RECV+"_"+(timeCurrent), Long.toString(receivedPackets));
					double avgDelay = receivedPackets == 0 ? 0 : (delaySum / receivedPackets)/Constants.SECOND;
					stats100.put(CBR_STAT_DELAY+"_"+(timeCurrent), Double.toString(avgDelay));
					float avgHopCount = receivedPackets == 0 ? 0 : hopCountSum / receivedPackets;
					stats100.put(CBR_STAT_HOPS+"_"+(timeCurrent), Float.toString(avgHopCount));
				}*/
			}
		}
	}
	 ...
	 */
	ExtendedProperties stats100 = new ExtendedProperties();
	public ExtendedProperties getStatsPer100() {
		return stats100;
	}
	...
	public ExtendedProperties getStats() {
		...
		stats.put(CBR_STAT_DELAY, Double.toString((double)avgDelay/(double)Constants.SECOND));
		float avgHopCount = receivedPackets == 0 ? 0 : hopCountSum / receivedPackets;
	...
	private static String[] getStatParameters() {
		return new String[] { CBR_STAT_SENT, CBR_STAT_RECV, CBR_STAT_DELAY,CBR_STAT_HOPS };
	}
